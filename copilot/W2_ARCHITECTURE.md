...
- **Cohere/dense rerank fallback path** (as of MVP cutoff 2026-05-05): BM25 only; FTS5 over SQLite. Architecture §5.5 / §10 already documented this deferral. **Post-MVP delta:** Reranker `Protocol` + Identity / Cohere / local-cross-encoder implementations shipped in Phase 2 (Early Submission, 2026-05-06, commit `22e59d28d`). Dense retrieval shipped in Phase 6 (2026-05-10, branch `feat/dense-retrieval`) — OpenAI `text-embedding-3-small` + RRF fusion behind the `COPILOT_DENSE_RETRIEVAL_ENABLED` kill-switch (default OFF). See `COST.md` §11 for cost / latency / fail-soft details.
